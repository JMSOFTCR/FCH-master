      ...
      <p class="card-text"></p>
         <div class="card-footer">
             <form action="details.php" method="post" name="Detalle">
                 ...
                 <input name="Detalles" type="submit" value="<?php echo $name; ?>" class="btn btn-success" />
             </form>
<!--              <a href="details.php?id=<?php echo $row['productid']; ?>" class="btn btn-success" style="text-align:center;"><?php echo $name; ?></a>-->
            </div>
    </div>
  </div>
	...
	?>
      ...
      <?php
          if($nro_pagina>1)
              echo "<a href='propane_buffers_parts.php?num=".($nro_pagina-1)."'> Anterior ></a> ";
              
       for ($i=1; $i<=$can_paginas; $i++){
           if ($i==$nro_pagina)
               echo $i." ";
           else 
               echo "<a href='propane_buffers_parts.php?num=$i'>$i</a> ";
       } 
          
          if($nro_pagina<$can_paginas)
               echo "<a href='propane_buffers_parts.php?num=".($nro_pagina+1)."'> Siguiente ></a> ";
