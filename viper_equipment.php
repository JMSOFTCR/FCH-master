...
<html lang="en">
  ...
  <body>
    ...
    include('conn.php');
     $query=mysqli_query($conn,"select * from product where categoryid = 15 order by product_name");  
    ...
    if(is_numeric($nro_pagina))
       ...
       $query=mysqli_query($conn,"select * from product where categoryid = 15 order by product_name limit $inicio,$reg_por_pagina");  
...
-->
	...
	?>
      ...
      <?php
          if($nro_pagina>1)
              echo "<a href='viper_equipment.php?num=".($nro_pagina-1)."'> Anterior ></a> ";
              
       for ($i=1; $i<=$can_paginas; $i++){
           if ($i==$nro_pagina)
               echo $i." ";
           else 
               echo "<a href='viper_equipment.php?num=$i'>$i</a> ";
       } 
          
          if($nro_pagina<$can_paginas)
               echo "<a href='viper_equipment.php?num=".($nro_pagina+1)."'> Siguiente ></a> ";
